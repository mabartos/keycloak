...
public class CustomJpaEntityProviderDistTest {
    ...
    @Test
    @Launch({"start-dev", "--db=dev-file", "--log-level=org.hibernate.jpa.internal.util.LogHelper:debug,org.keycloak.quarkus.deployment.KeycloakProcessor:debug", "--db-kind-new-user-store=dev-mem", "--db-kind-client-store=dev-file", "--db-kind-pu-without-dialect-store=dev-file"})
    void testUserManagedEntityNotAddedToDefaultPU(CLIResult cliResult) {
        ...
        cliResult.assertNoMessage("(JPA Startup Thread: pu-without-dialect-store) Error while creating file");
