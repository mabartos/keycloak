...
public class LoggingOptions {
    ...
    // Log format + tracing
    public static final Function<String, String> DEFAULT_LOG_FORMAT = (additionalFields) ->
            "%d{yyyy-MM-dd HH:mm:ss,SSS} " + (additionalFields.isEmpty() ? "" : additionalFields + " ") + "%-5p [%c] (%t) %s%e%n";

    public static final String DEFAULT_LOG_TRACING_FORMAT = DEFAULT_LOG_FORMAT.apply("traceId=%X{traceId}, parentId=%X{parentId}, spanId=%X{spanId}, sampled=%X{sampled}");
    ...
    public static final Option<String> LOG_CONSOLE_FORMAT = new OptionBuilder<>("log-console-format", String.class)
            ...
            .defaultValue(DEFAULT_LOG_FORMAT.apply(""))
            .build();
