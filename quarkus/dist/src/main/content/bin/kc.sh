...
if [ -z "$JAVA_OPTS" ]; then
   ...
   if [ -z "$JAVA_OPTS_KC_HEAP" ]; then
      ...
      if [ "$RUN_IN_CONTAINER" = "true" ]; then
         # Maximum utilization of the heap is set to 80% of the total container memory
         # Initial heap size is set to 20% of the total container memory
         JAVA_OPTS_KC_HEAP="-XX:MaxRAMPercentage=80 -XX:InitialRAMPercentage=20"
      fi
   else
