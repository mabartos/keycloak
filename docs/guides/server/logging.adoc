...
If the queue is already full, it either blocks until it is available or discards the log record, depending on the configuration.
...
If the queue is already full, it either blocks until it is available or discards the log record.
The default strategy is to **block** the main thread and wait for free space in the queue.

Possible values:

<@opts.expectedValues option="log-console-async-overflow"/>

You can change the strategy as follows:

<@kc.start parameters="--log-console-async-overflow=discard --log-file-async-overflow=block --log-syslog-async-overflow=block"/>

These properties are available only when asynchronous logging is enabled for these specific log handlers.

WARNING: You should be careful when changing the strategy to `discard` as {project_name} logs may contain potential crucial information that might be lost.
The recommended way is to use `discard` only for log handlers that might accept losing some log records.
