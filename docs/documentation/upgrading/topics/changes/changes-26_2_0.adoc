...
=== Deprecated properties in your `persistence.xml` file

When creating an additional JPA persistence unit in your deployment via `persistence.xml`, you should change the property name for specifying the datasource name.

Replace deprecated Hibernate property `hibernate.connection.datasource` with:

* `jakarta.persistence.jtaDataSource` (when `transaction-type="JTA"`)
