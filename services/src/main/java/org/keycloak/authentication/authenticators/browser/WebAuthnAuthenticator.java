...
import static org.keycloak.WebAuthnConstants.Events.ERR_DETAIL_LABEL;
import static org.keycloak.WebAuthnConstants.Events.ERR_LABEL;
...
public class WebAuthnAuthenticator implements Authenticator, CredentialValidator<WebAuthnCredentialProvider> {
    ...
    public void action(AuthenticationFlowContext context) {
        ...
        if (userHandle == null || userHandle.isEmpty()) {
            ...
            if (context.getUser() != null) {
                ...
                if (firstAuthenticatedUserId != null && !firstAuthenticatedUserId.equals(userId)) {
                    context.getEvent()
                            .detail(WebAuthnConstants.Events.FIRST_AUTHENTICATED_USER_ID, firstAuthenticatedUserId)
                            .detail(WebAuthnConstants.Events.AUTHENTICATED_USER_ID, userId);
        ...
        if (result) {
            ...
            context.getEvent()
                .detail(WebAuthnConstants.Events.USER_VERIFICATION_CHECKED, isUVChecked)
                .detail(WebAuthnConstants.Events.PUBKEY_CRED_ID_ATTR, encodedCredentialID);
            context.success();
        } else {
            context.getEvent()
                .detail(WebAuthnConstants.Events.AUTHENTICATED_USER_ID, userId)
