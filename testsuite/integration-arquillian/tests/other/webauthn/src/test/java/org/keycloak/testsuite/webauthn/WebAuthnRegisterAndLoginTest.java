    ...
    public void registerUserSuccess() throws IOException {
        ...
        try (RealmAttributeUpdater rau = updateRealmWithDefaultWebAuthnSettings(testRealm()).update()) {
            ...
            EventRepresentation eventRep = events.expectRequiredAction(CUSTOM_REQUIRED_ACTION)
                    ...
                    .detail(WebAuthnConstants.Events.PUBKEY_CRED_LABEL_ATTR, authenticatorLabel)
                    .detail(WebAuthnConstants.Events.PUBKEY_CRED_AAGUID_ATTR, ALL_ZERO_AAGUID)
                    .assertEvent();
            String regPubKeyCredentialId = eventRep.getDetails().get(WebAuthnConstants.Events.PUBKEY_CRED_ID_ATTR);
            ...
            sessionId = events.expectLogin()
                    .user(userId)
                    .detail(WebAuthnConstants.Events.PUBKEY_CRED_ID_ATTR, regPubKeyCredentialId)
                    .detail(WebAuthnConstants.Events.USER_VERIFICATION_CHECKED, Boolean.FALSE.toString())
                    .assertEvent().getSessionId();
    ...
    public void webAuthnPasswordlessAlternativeWithWebAuthnAndPassword() throws IOException {
        ...
        try (RealmAttributeUpdater rau = new RealmAttributeUpdater(testRealm())
            ...
            events.expectRequiredAction(CUSTOM_REQUIRED_ACTION)
                    ...
                    .detail(WebAuthnConstants.Events.PUBKEY_CRED_LABEL_ATTR, PASSWORDLESS_LABEL)
                    .assertEvent();

            events.expectRequiredAction(CUSTOM_REQUIRED_ACTION)
                    .user(userId)
                    .detail(Details.CUSTOM_REQUIRED_ACTION, WebAuthnRegisterFactory.PROVIDER_ID)
                    .detail(WebAuthnConstants.Events.PUBKEY_CRED_LABEL_ATTR, WEBAUTHN_LABEL)
                    .assertEvent();
