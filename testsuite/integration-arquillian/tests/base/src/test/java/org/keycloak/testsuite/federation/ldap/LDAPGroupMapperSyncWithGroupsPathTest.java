...
public class LDAPGroupMapperSyncWithGroupsPathTest extends AbstractLDAPTest {
    ...
    public void test01_syncWithGroupInheritance() {
        testingClient.server().run(session -> {
            ...
            Assertions.assertEquals(mapperModel.getConfig().getFirst("preserve.group.inheritance"), "true");
            ...
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group11"));
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group12"));
            ...
            Assertions.assertEquals("group12 - description", kcGroup12.getFirstAttribute(descriptionAttrName));
            ...
            Assertions.assertEquals("group1 - changed description", kcGroup1.getFirstAttribute(descriptionAttrName));
            Assertions.assertNull(kcGroup12.getFirstAttribute(descriptionAttrName));
        });
    }
    ...
    public void test02_syncWithDropNonExistingGroups() throws Exception {
        testingClient.server().run(session -> {
            ...
            // Assert groups are imported to keycloak including their inheritance from LDAP
            GroupModel kcGroup1 = KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1");
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/group11"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/group12"));
            ...
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/group11"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/group12"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/model1"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/model2"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, "/outside"));
            ...
            syncResult = new GroupLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToKeycloak(realm);
            Assertions.assertEquals(3, syncResult.getUpdated());
            Assertions.assertTrue(syncResult.getRemoved() == 2);

            // Sync and assert groups updated
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/group11"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/group12"));
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/model1"));
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(realm, LDAP_GROUPS_PATH + "/group1/model2"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(realm, "/outside"));
        });
    }
    ...
    public void test03_createConfigurationWithoutGroupPath() throws Exception {
        ...
        try {
            ...
            Assertions.assertEquals(GroupMapperConfig.DEFAULT_LDAP_GROUPS_PATH, newMapper.getConfig().getFirst(GroupMapperConfig.LDAP_GROUPS_PATH));
        } finally {
            // revert new mapper
