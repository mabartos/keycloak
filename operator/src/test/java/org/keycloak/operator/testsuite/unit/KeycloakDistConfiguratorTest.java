...
public class KeycloakDistConfiguratorTest {
    ...
    public void hostname() {
        final Map<String, String> expectedValues = Map.of(
                ...
                "hostname-url", "https://www.my-hostname.org:8448/something",
    ...
    public void missingHostname() {
        ...
        assertEnvVarNotPresent(envVars, "KC_HOSTNAME_URL");
        assertEnvVarNotPresent(envVars, "KC_HOSTNAME_PORT");
    ...
    @Test
    public void nonStandardHostnamePort() {
        final String message = "The default Ingress might not work with hostname-port set to non-standard value.";
        final Keycloak keycloak = K8sUtils.getResourceFromFile("test-serialization-keycloak-cr.yml", Keycloak.class);
        final StatefulSet deployment = getBasicKcDeployment();

        final Function<Integer, KeycloakStatus> setAndValidatePort = (port) -> {
            var hostnameSpec = keycloak.getSpec().getHostnameSpec();
            assertThat(hostnameSpec).isNotNull();

            hostnameSpec.setPort(port);

            assertThat(hostnameSpec.getPort()).isNotNull();
            assertThat(hostnameSpec.getPort()).isEqualTo(port);

            final KeycloakDistConfigurator distConfigPort = new KeycloakDistConfigurator(keycloak, deployment, null);
            final KeycloakStatusBuilder builder = new KeycloakStatusBuilder();
            distConfigPort.validateHostname(builder);
            return builder.build();
        };

        // Disabled default Ingress with specified port 8448
        assertThat(keycloak.getSpec().isDisableDefaultIngress()).isTrue();
        assertKeycloakStatusDoesNotContainMessage(setAndValidatePort.apply(8443), message);

        // Enabled default Ingress
        keycloak.getSpec().setDisableDefaultIngress(false);

        assertKeycloakStatusCondition(setAndValidatePort.apply(7777), KeycloakStatusCondition.HAS_ERRORS, false, message);
        assertKeycloakStatusDoesNotContainMessage(setAndValidatePort.apply(80), message);
        assertKeycloakStatusDoesNotContainMessage(setAndValidatePort.apply(443), message);
    }
    ...
    private void assertWarningStatusFirstClassFields(KeycloakDistConfigurator distConfig, boolean expectWarning, Collection<String> firstClassFields) {
        ...
        distConfig.validateFirstClassCitizens(statusBuilder);
